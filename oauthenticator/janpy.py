...
class JanpyEnvMixin(OAuth2Mixin):
    _OAUTH_ACCESS_TOKEN_URL = os.environ.get('OAUTH2_TOKEN_URL', '')
    _OAUTH_AUTHORIZE_URL = os.environ.get('OAUTH2_AUTHORIZE_URL', '')


class JanpyLoginHandler(OAuthLoginHandler, JanpyEnvMixin):
    pass


class JanpyOAuthenticator(OAuthenticator):
    ...
    login_handler = JanpyLoginHandler
    ...
    def authenticate(self, handler, data=None):
        ...
        if self.userdata_url:
            url = "{}?access_token={}".format(self.userdata_url, access_token)
        ...
        return resp_json.get(self.username_key);
        # return {
...
class LocalGenericOAuthenticator(LocalAuthenticator, JanpyOAuthenticator):
